...
# class DualLvTemAttenBlock_v1(nn.Module):
#     def __init__(self, channels, return_weights=False):
#         super().__init__()
...
#         frame_weights = torch.softmax(scores, dim=1)  # [B, 3, H, W]
...
#         # temporal attention block 추가
#         self.temporal_attn = DualLvTemAttenBlock_v2(num_feat)
...
#     def forward(self, x, return_intermediate=False):   # x: [b, t=3, c, h, w]
#         b, t, c, h, w = x.size()
        
#         # 인접 프레임 3개를 PixelUnshuffle & feat_extract → [b*t, c', h, w]
#         x = x.view(-1, c, h, w)
#         feat_0 = self.pus(self.feat_extract(x))   # [b*t, c', h', w']
        
#         # 다시 [b, t, c', h', w']로 재정렬
#         c_ = feat_0.size(1)
#         feat_0 = feat_0.view(b, t, c_, h // 2, w // 2)

#         # Temporal Attention(DLTA Block)으로 3프레임 융합
#         feat_0 = self.temporal_attn(feat_0)  # [B, C', H', W']
...
#         b, c, h, w = feat_l1.shape
#         feat_l1 = feat_l1.view(-1, 3, c // 3, h, w)
#         feat_l2 = feat_l2.view(-1, 3, c // 3, h // 2, w // 2)
#         feat_l3 = feat_l3.view(-1, 3, c // 3, h // 4, w // 4)
...
# @ARCH_REGISTRY.register()        
# class TemporalNet_v0(nn.Module):
#     def __init__(self, num_feat=64):
#         super(TemporalNet, self).__init__()
...
#         # Dual Level Temporal Attention Block
#         self.dlta_l1 = DualLvTemAttenBlock_v1(num_feat)
#         self.dlta_l2 = DualLvTemAttenBlock_v1(num_feat)
#         self.dlta_l3 = DualLvTemAttenBlock_v1(num_feat)
...
#         # feature extraction
#         feat_l1 = self.feat_extract1(feat_l1)  # [B*T, C, H, W]
#         feat_l2 = self.feat_extract2(feat_l2)
#         feat_l3 = self.feat_extract3(feat_l3)
...
#         # Apply DLTA Block to each scale
#         feat_l1_ref = self.dlta_l1(feat_l1)  # [B, C, H, W]
#         feat_l2_ref = self.dlta_l2(feat_l2)
#         feat_l3_ref = self.dlta_l3(feat_l3)
...
#     def __init__(self, num_feat=64):
#         super(TemporalNet_v1, self).__init__()
...
#         # Dual Level Temporal Attention Block v1
#         self.dlta_l1 = DualLvTemAttenBlock_v1(num_feat, return_weights=True)
#         self.dlta_l2 = DualLvTemAttenBlock_v1(num_feat, return_weights=True)
#         self.dlta_l3 = DualLvTemAttenBlock_v1(num_feat, return_weights=True)
...
#         # 프레임 정렬용 PCD Alignment 모듈
#         self.pcd_align = PCDAlignment(num_feat, deformable_groups=8)
        
#         # 정렬된 feature를 1개로 합성 conv
#         self.fusion = nn.Conv2d(num_feat * 3, num_feat, 3, 1, 1)
...
#         # DLTA 적용하여 attention-weighted fused feature 생성
#         feat_l1_ref, weight_l1 = self.dlta_l1(feat_l1)  # [B, C, H, W]
#         feat_l2_ref, weight_l2 = self.dlta_l2(feat_l2)
#         feat_l3_ref, weight_l3 = self.dlta_l3(feat_l3)
...
#         # 각 프레임별로 PCD alignment 수행
#         aligned_feat = []
#         for i in range(T):  # 세 프레임 반복
#             nbr_feat_l = [  # i번째 프레임의 3스케일 feature
# 	            feat_l1[:, i], feat_l2[:, i], feat_l3[:, i]
# 	          ]  
#             aligned_feat.append(self.pcd_align(nbr_feat_l, ref_feat_l))  # PCD 정렬

#         # 정렬된 feature들을 stack 후 융합
#         aligned_feat = torch.stack(aligned_feat, dim=1)      # [B, T, C, H, W]
#         aligned_feat = aligned_feat.view(B, -1, H, W)        # [B, T*C, H, W]
#         aligned_feat = self.leaky_relu(self.fusion(aligned_feat))  # [B, C, H, W]
