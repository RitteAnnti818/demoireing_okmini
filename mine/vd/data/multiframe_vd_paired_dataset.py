...
class MultiFrameVDPairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_gt_0 = read_img(gt_0_path)

        gt_1_path = self.paths[index]['gt_1_path']
        img_gt_1 = read_img(gt_1_path)

        gt_2_path = self.paths[index]['gt_2_path']
        img_gt_2 = read_img(gt_2_path)
        ...
        img_gt_0, img_gt_1, img_gt_2, img_lq_0, img_lq_1, img_lq_2 = img2tensor([img_gt_0, img_gt_1, img_gt_2, img_lq_0, img_lq_1, img_lq_2], bgr2rgb=True, float32=True)
        img_gts = torch.stack((img_gt_0, img_gt_1, img_gt_2), dim=0)
        img_lqs = torch.stack((img_lq_0, img_lq_1, img_lq_2), dim=0)

        return {'lq': img_lqs, 'gt': img_gt_1, 'gts': img_gts, 'key': key}
