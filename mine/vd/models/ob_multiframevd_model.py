...
#             # self.cri_pix_content = build_loss(train_opt['pixel_opt']).to(self.device)
#         else:
#             self.cri_pix = None
#             # self.cri_pix_content = None
...
#             out_dict['result'] = self.output['result'].detach().cpu()
#         else:
#             out_dict['result'] = self.output.detach().cpu()
#         out_dict['clean_img'] = self.clean_img.detach().cpu()
#         out_dict['content_img'] = self.content_img.detach().cpu()
#         out_dict['moire_img'] = self.moire_img.detach().cpu()
#         out_dict['content_mask'] = self.content_mask.detach().cpu()
#         out_dict['moire_mask'] = self.moire_mask.detach().cpu()
#         if hasattr(self, 'gt'):
...
#         # ✅ 변경 1: dict 구조로 받기
#         output_dict = self.net_d(self.lq)

#         # ✅ 변경 2: 분리된 요소 저장
#         self.content_img = output_dict['content_img']
#         self.moire_img = output_dict['moire_img']
#         self.content_mask = output_dict['content_mask']
#         self.moire_mask = output_dict['moire_mask']
        
#         # ✅ 기존 방식 유지 가능
#         self.l1 = output_dict['l1']
#         self.l2 = output_dict['l2']
#         self.l3 = output_dict['l3']
...
#         # if self.cri_pix_content:
#         #     l_content = self.cri_pix_content(self.content_img, self.gt)
#         #     l_total += l_content * 1.0  # <- 가중치 조절 가능
...
#     #         self.net_d_ema.eval()
#     #         with torch.no_grad():
#     #             output_dict = self.net_d_ema(self.lq)  # ✅ EMA 버전 사용
#     #             self.output = output_dict['clean_img']
#     #             self.content_img = output_dict['content_img']
#     #             self.moire_img = output_dict['moire_img']
#     #             self.content_mask = output_dict['content_mask']
#     #             self.moire_mask = output_dict['moire_mask']
...
#     #         self.net_d_ema.train()

#     #     else:
#     #         self.net_d.eval()
#     #         with torch.no_grad():
#     #             output_dict = self.net_d(self.lq)  # ✅ 일반 버전 사용
#     #             self.output = output_dict['clean_img']
#     #             self.content_img = output_dict['content_img']
#     #             self.moire_img = output_dict['moire_img']
#     #             self.content_mask = output_dict['content_mask']
...
#             self.net_t_ema.eval()
#             with torch.no_grad():
#                 d_out = self.net_d_ema(self.lq)  # dict
#                 clean_img = self.net_t_ema(d_out['l1'], d_out['l2'], d_out['l3'])  # tensor

#                 # 저장
#                 self.output = clean_img[:, :, :h_old * scale, :w_old * scale]
#                 self.clean_img = self.output
#                 self.content_img = d_out['content_img']
#                 self.moire_img = d_out['moire_img']
#                 self.content_mask = d_out['content_mask']
#                 self.moire_mask = d_out['moire_mask']

#             self.net_d_ema.train()
#             self.net_t_ema.train()

#         else:
#             self.net_d.eval()
#             self.net_t.eval()
#             with torch.no_grad():
#                 d_out = self.net_d(self.lq)  # dict
#                 clean_img = self.net_t(d_out['l1'], d_out['l2'], d_out['l3'])  # tensor
...
#         # # TemporalNet 출력을 위한 center frame 추출
#         # n, c, h, w = self.output.size()
#         # self.l1 = self.output.view(-1, 3, c, h, w)
#         # self.l1 = self.l1[:, 1, :, :, :].clone().squeeze(1)
